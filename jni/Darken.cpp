...
using namespace cv;
extern "C" jboolean Java_com_iqbal_deblurringapp_MainActivity_darkenImage(
		JNIEnv* env, jobject thiz, jint height, jint width, jintArray in,
		jintArray out) {
	...
	IplImage * mSrcGrayImg = cvCreateImage(cvGetSize(&mSrcImg), mSrcImg.depth,
			1);
	IplImage * mOutGrayImg = cvCreateImage(cvGetSize(&mSrcImg), mSrcImg.depth,
			1);
